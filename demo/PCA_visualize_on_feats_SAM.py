...
for img_path in  (img_filenames):
    ...
    # 提取特征
    with torch.no_grad():
        features = predictor.get_image_embedding().permute(0,2,3,1)[0,:35,:64,:]
        ...
        # 拟合数据并转换
        features = pca.fit_transform(features.reshape(-1,256))
        x = features.reshape(35, 64, 3)
        x = torch.tensor(x)
        x = x.permute(2,0,1)
        x = torch.nn.functional.interpolate(x.unsqueeze(0), (105, 192), mode='bilinear')[0]
        x = x.permute(1,2,0)
