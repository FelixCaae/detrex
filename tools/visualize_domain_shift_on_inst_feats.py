...
for label, ddir in enumerate(domain_dirs):
    ...
    for img_name, id_ in  zip(img_filenames, img_ids):
        ...
        # 提取特征
        with torch.no_grad():
            feature = model(img_tensor)['res5'].cpu()# 提取并压平特征向量
        features_list.append(feature.mean(dim=[2,3]).flatten().numpy())
        ...
        #     cy = bbox[1] + bbox[3]/2
        #     plt.plot()
        #     coord_rx = int((7 * cx / img_size[0]))
        #     coord_ry = int((7 * cy / img_size[1]))
...
plt.show()
plt.savefig('vis_out/inst_tsne.jpg')
plt.close()
...
plt.show()
plt.savefig('vis_out/inst_umap.jpg')
plt.close()