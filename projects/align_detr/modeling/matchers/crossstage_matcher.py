...
class CrossStageMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, match_num=0, layer_id =0):
        ...
        elif self.cost_class_type == "align_cost":
            out_prob = (
                outputs["pred_logits"].flatten(0, 1).sigmoid()
            )  # [batch_size * num_queries, num_classes]
        ...
        if self.cost_class_type == "ce_cost":
            ...
            gamma = self.gamma
            if len(tgt_bbox)>1:
                iou = box_iou(box_cxcywh_to_xyxy(out_bbox), box_cxcywh_to_xyxy(tgt_bbox))[0].max(dim=-1,keepdims=True)[0]
                q = iou 
            else:
                q =  out_prob
            neg_cost_class = (1-q) * (-(1 - out_prob + 1e-8).log())
            pos_cost_class = (q) * (-(out_prob + 1e-8).log())
            cost_class = pos_cost_class[:, tgt_ids] - neg_cost_class[:, tgt_ids]
