...
class DGDINO(nn.Module):
    ...
    def __init__(
        ...
        self.domain_embed = nn.Linear(embed_dim, 2)
        ...
        self.domain_embed = nn.ModuleList([copy.deepcopy(self.bbox_embed) for i in range(num_pred)])
        ...
        #     # nn.AdaptiveAvgPool2d((1, 1)),  # Use AdaptiveAvgPool for fixed output size
        #     nn.Flatten(),                  # Flatten before feeding to the MLP
        #     MLP(256, 256, domain_num, 3)           # Custom MLP with input_dim=1024 and output_dim=domain_num
        # )
    ...
    def adv_loss(self, adv_logits, domain_labels ):
        adv_loss_list = []
        for lvl, adv_logits_lv in enumerate(adv_logits):
            ...
            adv_loss = F.binary_cross_entropy_with_logits(adv_logits_lv, targets)
            adv_loss_list.append(adv_loss)

        return {'adv_loss':sum(adv_loss_list)}
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            for img_id in range(batch_size):
                ...
                # print(batched_inputs[img_id]['corrupt_idx'])
                domain_labels.append(batched_inputs[img_id]['corrupt_idx'])
        ...
        # for feat in inter_states:
        #     adv_logits_lvl = self.adv_classifier(grad_reverse(feat))
        #     adv_logits.append(adv_logits_lvl)
        ...
        for lvl in range(inter_states.shape[0]):
            ...
            outputs_domain_label = self.domain_embed[lvl](inter_states[lvl])
            ...
            outputs_domain_labels.append(outputs_domain_label)
        
        outputs_class = torch.stack(outputs_classes)
        # tensor shape: [num_decoder_layers, bs, num_query, num_classes]
        outputs_coord = torch.stack(outputs_coords)
       # tensor shape: [num_decoder_layers, bs, num_query, 4]
        outputs_domain_labels = torch.stack(outputs_domain_labels)
        # denoising postprocessing
        if dn_meta is not None:
            outputs_class, outputs_domain_labels, outputs_coord = self.dn_post_process(
                outputs_class, outputs_domain_labels, outputs_coord, dn_meta
            )

        # prepare for loss computation
        output = {"pred_logits": outputs_class[-1], "pred_boxes": outputs_coord[-1], 'pred_domains': outputs_domain_labels[-1]}
        if self.aux_loss:
        ...
        interm_domains = self.transformer.decoder.domain_embed[-1](enc_state)
        output["enc_outputs"] = {"pred_logits": interm_class, "pred_boxes": interm_coord, 'pred_domains':interm_domains}
        if self.training:
            ...
            # adv_loss_dict = self.adv_loss(adv_logits, domain_labels)
    ...
    def dn_post_process(self, outputs_class, outputs_domain_labels, outputs_coord, dn_metas):
        if dn_metas and dn_metas["single_padding"] > 0:
            ...
            outputs_domain_labels = outputs_domain_labels[:,:, padding_size:, :]
        ...
        return outputs_class, outputs_coord, outputs_domain_labels
